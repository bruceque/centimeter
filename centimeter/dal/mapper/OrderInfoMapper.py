...
class OrderInfoMapper(object):
	...
	def selectBatch(self, start, number):
		...
		return self.session.query(OrderInfoDO).filter(OrderInfoDO.isDeleted=="N").\
				   ...
				   filter(OrderInfoDO.sellerId==1).filter(OrderInfoDO.shopId.notin_([16507,296068,137035,274225,296066,136686,291857,23,291857,18,132497,86290,177427,278555,56,12943,120220,86290,177427,40631,274629,278555,290801,234238,309906,67627,287302,306080,318108])).\
				   filter(OrderInfoDO.orderSn.notilike("%S%")).filter(OrderInfoDO.orderSn.notilike("%Y%")).filter(OrderInfoDO.orderSn.notilike("%XJ%")).filter(OrderInfoDO.orderAmount>0).order_by(OrderInfoDO.id)[start:start+number]
